...
class OpenfedValidations {
  ...
  private static function _checkTwigTweak2Compatibility() {
    if (self::_isTwigTweakEnabled()) {
      ...
      $search = shell_exec('find ./docroot/themes/ ./config/ -name "*.twig" | xargs grep "drupal_block"');
      ...
      $search = shell_exec('find ./docroot/themes/ ./config/ -name "*.twig" | xargs grep "preg_replace"');
      ...
      $search = shell_exec('find ./docroot/themes/ ./config/ -name "*.twig" | xargs grep "drupal_set_message"');
